...
function inferPricingScope(sourceId) {
  switch (sourceId) {
    case "target-public":
      return "online_generic";
    case "kroger":
    case "qfc":
    case "fred-meyer":
      return "store_level";
...
export async function buildProviderCapabilitiesReport() {
  ...
  const reports = sources.map((source) => {
    ...
    return {
      ...
      availabilityAvailable,
      storeLevelPricingAvailable,
      pricingScope: inferPricingScope(source.id),
      supportsSearch,
      supportsPdp,
      latestObservedAt,
      latestError,
...
export function printProviderCapabilitiesSummary(report) {
  console.log("sourceId".padEnd(18), "status".padEnd(8), "scope".padEnd(14), "price".padEnd(8), "availability".padEnd(15), "latestError");
  for (const provider of report.providers) {
    const price = provider.priceAvailable ? "price" : "no-price";
    const availability = provider.availabilityAvailable ? "availability" : "no-availability";
    console.log(`${provider.sourceId.padEnd(18)} ${provider.status.padEnd(8)} ${String(provider.pricingScope).padEnd(14)} ${price.padEnd(8)} ${availability.padEnd(15)} ${provider.latestError?.message ?? "ok"}`);
  }
}
