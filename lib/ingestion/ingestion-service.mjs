...
function buildHistoricalPriceRecord(product, basketItem = null) {
  ...
  return {
    ...
    pricing_scope: product.pricingScope ?? "unknown",
    price: product.price,
...
export async function validateSources({ enabledOnly = true } = {}) {
  ...
  for (const source of sources) {
    try {
      if (source.id === "walmart" || source.id === "kroger-public") {
        ...
        if (!validation.ok) {
          const errorType = validation.reason === "blocked_by_robots"
            ? ErrorTypes.blockedByRobots
            : ErrorTypes.invalidResponse;
          errors.push(createSourceError(errorType, source.id, { validation }));
        }
        continue;
      }
...
export async function ingestProducts({ sourceId, zipCode, query, storeId, productUrl, itemId }) {
  const run = {
    id: createId("run"),
  ...
  const requiresStoreContext = ["kroger", "qfc", "fred-meyer"].includes(sourceId);

  if (!resolvedStoreId && zipCode && requiresStoreContext) {
    try {
      const stores = await ingestKrogerStoresForZip(sourceId, zipCode);
  ...
  if (requiresStoreContext && !resolvedStoreId) {
    const finishedRun = {
      ...run,
      completedAt: nowIso(),
      status: "failed",
  ...
  try {
    ...
    const errorType = error.message?.includes("robots.txt disallows")
      ? ErrorTypes.blockedByRobots
      : ErrorTypes.invalidResponse;
    errors.push(createSourceError(errorType, sourceId, {
      query,
      storeId: resolvedStoreId,
      productUrl,
      itemId,
  ...
  const responseSelectedStore = response.selectedStore ?? null;
  const effectiveStoreId = resolvedStoreId ?? responseSelectedStore?.storeId ?? null;
  const effectiveStoreName = resolvedStoreName ?? responseSelectedStore?.name ?? null;

  if (responseSelectedStore?.storeId) {
    await upsertById(outputFiles.storesDb, [{
      id: responseSelectedStore.storeId,
      retailerSourceId: sourceId,
      externalStoreCode: responseSelectedStore.storeId,
      name: responseSelectedStore.name ?? sourceId,
  ...
  const products = response.products.map((rawProduct) =>
    trimRawForDisk(
      adapter.normalizeProduct(rawProduct, {
        retailer: sourceId === "kroger"
          ? "Kroger"
          : sourceId === "qfc"
            ? "QFC"
            : sourceId === "fred-meyer"
              ? "Fred Meyer"
              : adapter.source.retailerName,
      ...
      try {
        ...
        await Promise.all([
          upsertById(outputFiles.productsDb, [{
            ...
            pricing_scope: topProduct.pricingScope ?? "unknown",
