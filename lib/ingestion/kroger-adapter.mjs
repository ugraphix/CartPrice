...
function getCredentialStatus() {
  return {
    clientIdSet: Boolean(krogerConfig.clientId),
    clientSecretSet: Boolean(krogerConfig.clientSecret),
  };
}
...
async function getAccessToken() {
  ...
  if (!krogerConfig.clientId || !krogerConfig.clientSecret) {
    throw new Error("Kroger credentials not set. Live ingestion will fail.");
  }
  ...
  let response;
  try {
    response = await fetchWithPolicy("https://api.kroger.com/v1/connect/oauth2/token", {
      method: "POST",
      headers: {
    ...
    if (error instanceof HttpError && error.details.status === 401) {
      throw new Error(
        "Kroger auth failed (401). Likely invalid client_id/client_secret or incorrect OAuth scope.",
      );
    }
    throw error;
  }
...
export class KrogerAdapter extends BaseSourceAdapter {
  ...
  async validateSourceAccess() {
    const env = getCredentialStatus();
    if (!env.clientIdSet || !env.clientSecretSet) {
      return {
        ok: false,
