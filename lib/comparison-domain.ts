import { normalizeText, stringSimilarity, tokenize } from "./fuzzy.ts";
import type {
...
export function getCurrentLocalParts(timezone: string) {
...
function activeRule<T extends PricingRule["ruleType"]>(pricingRules: PricingRule[], ruleType: T) {
...
function inferMatchMethod(score: number, product: ComparableCatalogProduct, item: ShoppingListItem) {
  if (product.upc && item.preferredProductName && stringSimilarity(item.preferredProductName, product.name) > 0.98) {
    return "upc";
  }
  if (score >= 0.92) {
    return "exact";
...
export function matchScore(item: ShoppingListItem, product: ComparableCatalogProduct) {
...
export function matchItemsForStore(storeId: string, list: ShoppingListItem[], catalogProducts: ComparableCatalogProduct[]): MatchResult[] {
  ...
  return list.map((item) => {
    ...
    const chosenPrice = best.product.price ?? 0;
    return {
      item,
      ...
      reason:
        best.product.stalePrice
          ? "Matched product found, but the latest observed price is stale."
          : best.score < 0.8
            ? "Estimated match based on text and size similarity."
            : "Strong catalog match.",
      lineTotal: chosenPrice * item.quantity,
      matchMethod: inferMatchMethod(best.score, best.product, item),
...
function calculateSalesTax(matches: MatchResult[], pricingRules: PricingRule[]) {
  const salesTaxRule = activeRule(pricingRules, "sales_tax");
  const rate =
...
function calculateBeverageTax(matches: MatchResult[], pricingRules: PricingRule[]) {
  const beverageRule = activeRule(pricingRules, "sweetened_beverage_tax");
  ...
  return matches.reduce((sum, match) => {
    ...
    const ounces = product.sizeUnit === "oz" ? (product.sizeValue ?? 0) * (product.packageCount ?? 1) : 0;
    return sum + ounces * beverageRule.ruleJson.ratePerOunce * match.item.quantity;
  }, 0);
}

function calculateBagFee(matches: MatchResult[], pricingRules: PricingRule[]) {
  const bagRule = activeRule(pricingRules, "bag_fee");
...
export function compareStoreCatalog(params: {
  stores: Store[];
  ...
  const nearbyResults = params.stores
    ...
    .map(({ store, distanceMiles }) => {
      ...
      const beverageTax = calculateBeverageTax(matches, params.pricingRules);
      const bagFee = calculateBagFee(matches, params.pricingRules);
